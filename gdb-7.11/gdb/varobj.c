	       ...
	       char *expression, CORE_ADDR frame, enum varobj_type type)
{
  ...
  if (expression != NULL)
    {
      if (hsail_print_is_hsail_expression(expression))
        {
          struct value *hsail_value = NULL;
          char* printFormat=(char*)malloc(10);
          size_t format_size = 0;
          printFormat[0] = 0;

          /* This is a temporary workaround till we surface the HSAIL
           * function call for the corresponding variable. */
          var->root->exp = parse_expression ("RunExpressionEval()");

          /* Get the value for the ROCm register */
          hsail_value = hsail_print_expression(expression, printFormat, &format_size);

          if (hsail_value != NULL)
            {
              int real_type_found = 0;

              var->type = value_actual_type (hsail_value, 0, &real_type_found);
              if (real_type_found)
                {
                  hsail_value = value_cast (var->type, hsail_value);
                }

              /* Set types based on encoding returned by debug facilities */
              if (printFormat[0] == 'f' && 4 == format_size)
                {
                  var->type->length = 4;
                  var->type->main_type->code = TYPE_CODE_FLT;
                  var->type->main_type->name = "float";
                }
              else if (printFormat[0] == 'u' || printFormat[0] == 'd')
                {
                  var->type->length = 4;
                  var->type->main_type->code = TYPE_CODE_INT;
                  var->type->main_type->name = "int";
                }
            }

          /* Populating the var structure is done the same way non-ROCm case.
           * We dont need to do any of the frame stuff since we don't
           * have any frame information for ROCm */
          ...
          install_new_value (var, hsail_value, 1 /* Initial assignment */);
  ...
  if (expression != NULL && 0 == (hsail_print_is_hsail_expression(expression)))
    {
