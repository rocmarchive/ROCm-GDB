...
create_breakpoint_hsail(struct gdbarch *gdbarch,
                        const char *arg, char* extra_string, int internal,
                        const struct breakpoint_ops *ops)
{
  struct breakpoint *b = NULL;
  struct ui_out *uiout = current_uiout;
...
/* #define HSAIL_PC_BP */
  ...
  if (hsail_breakpoint_parse_bp_request(arg, extra_string, &hsail_bp_request))
  {
...
/*
 * We need to be sure that resetting GPU breakpoint is only called once per run of the inferior
    ...
    if (b->type == bp_hsail)
      {
        ...
        if(b->hsail_bp_request == NULL)
          {
            printf_filtered("Reset GPU breakpoints in post-create-inferior\n");
            printf_filtered("GPU Breakpoint %d \t with a NULL Request\n",b->number);
          }
          ...
          case HSAIL_BP_TYPE_UNKNOWN:
            {
               ...
               * */
              /*printf_filtered("Unknown GPU breakpoints in post-create-inferior\n");*/
            }
            break;
          default:
             ...
             * */
            printf_filtered("Uninitialized  GPU breakpoints: breakpoint_re_set_all_hsail\n");
            gdb_assert(0);
            break;
        }
      }
		   ...
		   unsigned flags)
{
  ...
  if (is_hsail_breakpoint(args))
    {
      create_breakpoint_hsail(gdbarch,args,extra_string,internal,ops);
      return 1;
    }
...
{
  struct breakpoint *b = NULL;
  const struct bp_location *bl = NULL;
  int bp_temp = 0;
  struct ui_out *uiout = current_uiout;
  ...
  if (bl->address != bl->requested_address)
    breakpoint_adjustment_warning (bl->requested_address,
                   ...
                   b->number, 1);
  annotate_breakpoint (b->number);
  hsail_breakpoint_print_stopped_reason();
  if (bp_temp)
    ui_out_text (uiout, "Temporary breakpoint triggered by GPU Kernel Breakpoint\n");
  else
    ui_out_text (uiout, "Stopped on GPU breakpoint\n\n");
  if (ui_out_is_mi_like_p (uiout))
    {
      ui_out_field_string (uiout, "reason",
            async_reason_lookup (EXEC_ASYNC_BREAKPOINT_HIT));
...
delete_breakpoint (struct breakpoint *bpt)
{
  ...
  if(hsail_rc == 0)
    {
      printf_filtered("GPU breakpoint may not have been deleted safely\n");
    }
...
create_hsa_gpu_breakpoint_trigger(const char *location)
{
   ...
   * char *.
   */
  int len = strlen(location) + 1;
  char* pStrCpy = NULL;
  struct event_location* elocation;
  struct cleanup *cleanup;

  if (NULL != gs_pGpuBpStr)
  {
    free(gs_pGpuBpStr);
  }
  gs_pGpuBpStr = malloc(len);
  gdb_assert(NULL != gs_pGpuBpStr);

  pStrCpy = gs_pGpuBpStr;

  memset(gs_pGpuBpStr, '\0', len);
  strncpy(gs_pGpuBpStr, location, len-1);
  elocation = string_to_event_location(&pStrCpy, current_language);
  cleanup = make_cleanup_delete_event_location(elocation);
  create_breakpoint(get_current_arch(),
            elocation,
            NULL, 0, NULL, 1,
            0, bp_breakpoint,
...
void
delete_hsa_agent_internal_breakpoint(void)
{
  struct breakpoint* b = NULL;
  struct breakpoint* b_tmp = NULL;
  int isBpFound = 0;

  if (NULL == gs_pGpuBpStr)
  {
    /* The internal breakpoint already deleted. */
    return;
  }
  ...
  {
      if (b->type == bp_breakpoint
          && user_breakpoint_p(b) == 0
          && (NULL != b->location) && (NULL != b->location->as_string)
          && strcmp(b->location->as_string, gs_pGpuBpStr) == 0)
        {
          isBpFound = 1;
          delete_breakpoint(b);
        }
...
Set breakpoint at specified location. \nThis command can be used for GPU breakpoints and CPU breakpoints\n"
HSAIL_BREAK_HELP()
