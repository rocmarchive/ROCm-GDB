...
quit_command (char *args, int from_tty)
{
  ...
  /* Kill HSA GPU dispatches. */
  hsail_tdep_kill_all_waves(true);
...
list_command (char *arg, int from_tty)
{
  ...
  if (hsail_is_focus_device() && is_hsail_thread_focus_thread)
    {
...
disassemble_command (char *arg, int from_tty)
{
  ...
  if(hsail_is_focus_device() || hsail_is_focus_predispatch())
    {
      if (hsail_print_gpu_disassembly(arg))
        {
          return;
        }
      else
        {
          printf_filtered("GPU ISA could be found, fall back to default behavior\n");
        }
    }
...
can be shown using \"show listsize\"."));
  ...
  c = add_com ("disassemble", class_vars, disassemble_command, _(HSAIL_DISASSEMBLE_HELP()"\
