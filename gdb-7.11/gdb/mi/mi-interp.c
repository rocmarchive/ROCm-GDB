...
mi_on_signal_received (enum gdb_signal siggnal)
{
  ...
  /**
   * Note: Perhaad (6/15/16), originally the function call below was called always
   *
   * print_signal_received_reason (mi->cli_uiout, siggnal);
   *
   * Now it is called for all cases other than the HSAIL breakpoint case.
   * The motivation for this is below.
   */

  /* This is a less than elegant solution to the problem of how the GDB hit
   * message for HSAIL breakpoints seems to be printed multiple times on
   * the stdout when running with (gdb -i=mi).
   *
   * There is no good way to handle the difference between CLI and MI in the
   * lower level functions. Even if we had two totally different GPU hit
   * breakpoint printing functions, both would be called by this function.
   *
   * I am unsure if this has any effect when ROCm-gdb may drive some
   * GUI but we can work on finding a solution when the problem turns up.
   * */
  if (!hsail_is_signal_from_agent(gdb_signal_to_string(siggnal)))
    {
      print_signal_received_reason (mi->cli_uiout, siggnal);
    }
}
