...
sighsail_handler (int signo)
{
  ...
  ui_out_text(uiout, "[ROCm-gdb: <Handling a SIGALRM - Used for GPU Debugging>]\n");
...
/* Called for process shut down.
 *
 * GPU dispatch things that persist across inferior runs such as
 * the breakpoint cache should not be cleared here
 * */
static void
