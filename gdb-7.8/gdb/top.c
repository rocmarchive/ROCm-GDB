...
#define DEFAULT_PROMPT	"(ROCm-gdb) "
#endif
...
{
  char* env_var = getenv("ROCM_ENABLE_GDB");

  if (env_var == NULL)
    {
      printf_filtered("\n\nGDB has detected that it is not being run using the rocm-gdb.\n");
      printf_filtered("Debugging GPU dispatches may not work\n");
      printf_filtered("Please run rocm-gdb instead.\n\n\n");
      exit(1);
    }
}
...
and \"show warranty\" for details.\n");
  ...
  fprintf_filtered (stream, "AMD rocm-gdb %s\n", hsail_gdb_version);
...
resources online at:\n<http://www.gnu.org/software/gdb/documentation/>.\n"));
  ...
  fprintf_filtered (stream, _("For ROCm GPU debugging specific help, type \"help rocm\".\n"));
