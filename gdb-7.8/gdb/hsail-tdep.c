...
void* hsail_tdep_map_isa_buffer(void)
{
  ...
  shmid = shmget(g_ISASTREAM_SHMKEY,
                 g_ISASTREAM_MAXSIZE, 0666);

  if (shmid <= 0)
    {
      ui_out_text(uiout, "ISA buffer mapping: shmid is invalid\n");
    }
...
bool hsail_tdep_save_isa(bool is_disassemble_command, const char* hsail_isa_file_name)
{
  ...
  if(hsail_cmd_get_show_isa_option() == true || is_disassemble_command)
    {
      size_t isa_size;
      pIsaShm = hsail_tdep_map_isa_buffer();

      gdb_assert(pIsaShm!= NULL);

      /* No isa found*/
      isa_size =  *((size_t*)pIsaShm);
      if (isa_size ==  0)
        {
          hsail_tdep_unmap_shm_buffer(pIsaShm);
          return false;
        }

      /* Skip over size*/
      isa_location = (char*)((size_t*)pIsaShm + 1);
