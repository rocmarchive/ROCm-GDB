  ...
  if (config.trace_file_handle == NULL)
    {
      if(config.trace_file_name != NULL)
        ...
        {
          printf_filtered("Invalid file name\n");
...
static void hsail_trace_set_file_name(const char* ip_option, const bool set_random)
{
  ...
  if(set_random)
  ...
  else if (ip_option != NULL)
    {
      ...
      if (sanitized_ip_option != NULL)
        {
          ...
          if (config.trace_file_name != NULL)
            {
              int err_no = 0;
              err_no = rename(config.trace_file_name, sanitized_ip_option);
              if( err_no != -1)
                {
                  printf_filtered("[ROCm-gdb]: Rename the kernel launch file \"%s\" to \"%s\"\n",
            ...
            {
              /* No file name existed, save the sanitized user input */
              hsail_utils_copy_string(&(config.trace_file_name), sanitized_ip_option);
            }
      ...
      printf_filtered("[ROCm-gdb]: GPU kernel launch trace will be saved to %s\n", config.trace_file_name);
    }
  ...
  if(strcmp(ip_option,"on") == 0)
    {
      ...
      printf_filtered("GPU Kernel launch tracing has been enabled\n");
      printf_filtered("GPU Kernel launch trace will be saved to %s\n", config.trace_file_name);
